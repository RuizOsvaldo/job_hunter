...
def _parse_salary(row) -> tuple[int | None, int | None]:
    ...
    def _safe_int(v):
        return int(v) if v is not None and v == v else None  # v == v is False for NaN

    return _safe_int(lo), _safe_int(hi)
...
def _scrape_one(term: str, location: str) -> list[dict]:
    """Run a single jobspy search. Returns list of job dicts ready for upsert."""
    try:
        df = scrape_jobs(
...
def run_search() -> int:
    """Run all configured searches in parallel. Returns count of new jobs inserted."""
    searches = [
        (term, location)
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        futures = {pool.submit(_scrape_one, term, loc): (term, loc) for term, loc in searches}
        for future in as_completed(futures):
            ...
            try:
                ...
                for job in jobs:
                    if upsert_job(job):
                        new_count += 1
                        print(f"[scraper] NEW: {job['title']} @ {job['company']} — {job['work_type']}, {job['city']}, {job['state']}")
            except Exception as e:
                print(f"[scraper] Unhandled error for '{term}' @ '{loc}': {e}")
