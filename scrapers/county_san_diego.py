"""County of San Diego scraper — governmentjobs.com/careers/sdcounty."""
import hashlib
import logging
import time
...
_BASE_URL = "https://www.governmentjobs.com/careers/sdcounty"
_API_URL = "https://www.governmentjobs.com/careers/sdcounty/jobs"
_DAYS_BACK = 5
_KEYWORD = "analyst"
...
def _parse_salary(job: dict) -> tuple[int | None, int | None]:
    ...
    if not salary_str:
        return None, None
    # Format typically: "$XX,XXX.XX - $XX,XXX.XX Annually"
    import re
...
def scrape_county_san_diego() -> list[dict]:
    """Scrape County of San Diego jobs for analyst roles (last 5 days)."""
    ...
    try:
        ...
        logger.warning("[county_sd] Request failed: %s", exc)
        return []

    # governmentjobs.com returns items under various keys depending on endpoint
    items = (
        data.get("jobs")
    ...
    logger.info("[county_sd] %d analyst jobs found", len(jobs))
